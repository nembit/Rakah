...
export const sendLogsToRemote = async (logs) => {
  if (!process.env.EXPO_PUBLIC_LOGS_ENDPOINT || !process.env.EXPO_PUBLIC_LOGS_API_KEY) {
...
export const reportErrorToRemote = async ({ error }) => {
  if (!process.env.EXPO_PUBLIC_LOGS_ENDPOINT || !process.env.EXPO_PUBLIC_LOGS_API_KEY) {
    console.debug('reportErrorToRemote: Missing logging endpoint or API key.', error);
    return { success: false };
  }
